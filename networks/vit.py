...
class BaseViT(BaseModule):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000,
        ...
        print('init channel_conv')
        self.channel_conv = nn.Conv2d(in_chans, 3, kernel_size=1)
        print('TIMMViT __init__ done')
    ...
    def init_weights(self, pretrained=None):
        if isinstance(pretrained, str):
            ...
            # self.load_from(weights=np.load(pretrained))
        # print('not used')
        else:
            return
    ...
    def forward_features(self, x):
        x = self.channel_conv(x) # channel 4 -> 3
