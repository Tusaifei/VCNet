...
from .vit import BaseViT
from .FuseX_modules import CNN, CTIBlock, deform_inputs

_logger = logging.getLogger(__name__)


class ViT_FuseX(BaseViT):
    def __init__(self, pretrain_size=224, num_heads=12, conv_inplane=64, n_points=4, deform_num_heads=6,
                ...
                #  in_chans=3,
                 *args, **kwargs):
        print('ViT_FuseX __init__', kwargs)
        ...
        self.spm = CNN(inplanes=conv_inplane, embed_dim=embed_dim, in_chans=kwargs['in_chans'])
        self.spm2 = CNN(inplanes=conv_inplane, embed_dim=embed_dim, in_chans=64)
    ...
    def forward(self, x):
        ...
        if isinstance(x, tuple):
            use_L2HNet = True
            # print('received input from L2HNet')
            x, cnn_feat = x
            deform_inputs1, deform_inputs2 = deform_inputs(cnn_feat)
            c1, c2, c3, c4 = self.spm2(cnn_feat)
            H = W = cnn_feat.shape[-1] // 16
            bs, n, dim = x.shape
        else:
            ...
            # print('shape before patch_emb:', x.shape)
            x, H, W = self.patch_embed(x)  # [B, C, H, W] -> [B, C, HW] -> [B, HW, C]
            bs, n, dim = x.shape # 2, 196, 192
            pos_embed = self._get_pos_embed(self.pos_embed[:, 1:], H, W)
            x = self.pos_drop(x + pos_embed)
            # print('shape after patch_emb:', x.shape)
        ...
        if use_L2HNet:
            if self.add_vit_feature:
                # seperate and feed to 2 loss funcs
                x1, x2, x3, x4 = outs
