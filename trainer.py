...
def trainer_dataset(args, model, snapshot_path):
    ...
    for epoch_num in iterator:
        ...
        for i_batch, (image_batch,label_batch) in tqdm(enumerate(trainloader),  total=num_training_batches_per_epoch):
            ...
            comer_out, cnn_out = model(image_batch)
            loss_ce_cnn = ce_loss(cnn_out, label_batch[:].long())

            t_cnn = F.softmax((cnn_out), dim=1) # Created mask label
            t_cnn = t_cnn.argmax(axis=1)
            mask_cnn = torch.where(t_cnn==label_batch, label_batch, 0)
            loss_ce_comer = ce_loss(comer_out, mask_cnn[:].long())

            loss = 0.5 * loss_ce_cnn + 0.5 * loss_ce_comer
            ...
            for param_group in optimizer.param_groups:
                param_group['lr'] = lr_
            loss1.append(loss_ce_cnn.item())
            loss2.append(loss_ce_comer.item())
            iter_num = iter_num + 1
